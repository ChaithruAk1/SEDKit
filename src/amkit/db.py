...
from amkit.errors import Busy, PreconditionFailed, ValidationFailed

BUSY_TIMEOUT_MS = 15_000
SCHEMA_PACKAGE = "amkit.schema"
...
def connect(db_path: Path, *, readonly: bool = False) -> sqlite3.Connection:
    ...
    if readonly and not db_path.exists():
        raise PreconditionFailed(f"Database not found: {db_path}. Run `amkit init` first.")
    ...
    * The version is re-read under the write lock, so concurrent migrators do not re-apply a script.
    ...
    if current > latest:
        raise PreconditionFailed(
            f"Database schema v{current} is newer than this amkit (v{latest}); upgrade the code instead.",
        )
...
def backup(
    ...
    *,
    ...
    target = backups_dir / f"amkit-{stamp}-{safe_reason}.db"
...
def prune_backups(backups_dir: Path, *, keep: int, protect: tuple[Path, ...] = ()) -> list[Path]:
    ...
    files = sorted(backups_dir.glob("amkit-*.db"), key=lambda p: p.name, reverse=True)
    removed = []
...
def last_backup_age_hours(backups_dir: Path) -> float | None:
    files = sorted(backups_dir.glob("amkit-*.db"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not files:
        return None
...
def serve_lock_holder(lock_path: Path) -> int | None:
    """PID of a live `amkit serve` holding the lock, else None (stale locks are ignored)."""
...
def inspect_backup(backup_file: Path) -> dict[str, Any]:
    """Open a candidate backup read-only (never creates a file) and validate it is an amkit database."""
    ...
    if not has_meta or not (0 < version <= latest_version()):
        raise PreconditionFailed(
            f"{backup_file} is not an amkit database this version can restore (schema v{version})."
        )
...
def restore(
    ...
    *,
    ...
    if holder is not None:
        raise PreconditionFailed(f"`amkit serve` is running (pid {holder}); stop it before restoring.")
    ...
    with tempfile.TemporaryDirectory(prefix="amkit-restore-") as tmp:
