"""`amkit doctor`: environment and profile health checks. Any `fail` makes the command exit 4.
...
from amkit.errors import AmkitError
from amkit.paths import Paths, data_root, enclosing_git_tree, is_unc, is_under_onedrive, repo_root
from amkit.salt import fingerprint, read_salt
from amkit.settings import PII_MODES, load_agent_config, load_settings
...
def run_checks(paths: Paths, *, skip: set[str] | None = None) -> list[Check]:
    ...
    add(_check("skill_names_valid", not bad_names, ", ".join(bad_names) or "all am-* with matching frontmatter name"))
    ...
    if paths.data_class == "real":
        add(
            _check(
                ...
                "record with `amkit init --profile real --ai-approval-note ...`",
                severity="warn",
            )
        )
    return checks
...
def _database_checks(paths: Paths, add) -> dict[str, str]:
    ...
    add(_check("db_exists", db_exists, str(paths.db) if db_exists else "run `amkit init`"))
    if not db_exists:
        return {}
    meta: dict[str, str] = {}
    try:
...
def _salt_checks(paths: Paths, meta: dict[str, str], add) -> None:
    try:
        ...
        return
    if salt is None:
        hint = "restore the original salt file" if meta.get("salt_fingerprint") else "run `amkit init --new-salt`"
...
def _claude_checks(paths: Paths, add) -> None:
    ...
    try:
        ...
        add(
            _check(
                "claude_settings_local", runs_ok, f"{local} lists {paths.runs}" if runs_ok else "re-run `amkit init`"
            )
        )
    add(
        _check(
...
def skill_name_problems(root: Path) -> tuple[list[str], list[str]]:
    """(clashes with personal skills, folders that are not am-* or whose frontmatter name differs)."""
    ...
    for folder in sorted(p for p in skills_dir.iterdir() if p.is_dir()):
        if not folder.name.startswith("am-"):
            bad.append(f"{folder.name} (not am- prefixed)")
