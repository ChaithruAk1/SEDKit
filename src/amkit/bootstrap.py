"""`amkit init`: create a profile's DATA_DIR, database, meta, salt and Claude Code wiring."""
...
from amkit import __version__, claude_setup, db
from amkit.errors import PreconditionFailed, ValidationFailed
from amkit.paths import Paths
from amkit.salt import create_salt, fingerprint, read_salt
    ...
    *,
    ...
    try:
        ...
        with db.write_tx(conn):
            ...
            db.set_meta(conn, "amkit_version", __version__)
    ...
    if salt is None and final_meta.get("salt_fingerprint"):
        ...
        warnings.append("No PII salt yet: run `amkit init --new-salt` once for this new profile.")
