"""PII salt: created only by `sed init --new-salt`, never auto-generated.
...
# Frozen protocol constant from before the SED rename: changing it would invalidate every stored salt fingerprint.
...
def require_salt(salt_file: Path, expected_fingerprint: str | None) -> bytes:
    ...
    if salt is None:
        raise PreconditionFailed(
            ...
            "(or, for a brand-new profile only, run `sed init --new-salt`).",
        )
