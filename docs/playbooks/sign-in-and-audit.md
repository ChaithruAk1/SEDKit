...
Phase 1 (identity) is built: `docs/sign-in.md`. The audit log goes in its own file under `DATA_DIR`, not in `sed.db`,
because `sed db restore` replaces `sed.db` wholesale and GET routes must never write it.
